...
public class Captain extends NPC {
    private Animation<TextureRegion> captainAnimation;
    private Array<TextureRegion> frames;
    private float stateTimer;
    private TextureRegion region;

    public Captain(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        setBounds(getX(), getY(), 56 / Constants.PPM, 48 / Constants.PPM);
        frames = new Array<TextureRegion>();
        for (int i = 1; i <= 32; i++) {
            frames.add(new TextureRegion(Utils.getRegion("npc/Captain/" + i + ".png")));
        }
        captainAnimation = new Animation<TextureRegion>(0.4f, frames);
        stateTimer = 0;
    }
    ...
    public void update(float dt) {
        super.update(dt);
        stateTimer += dt;
        region = captainAnimation.getKeyFrame(stateTimer, true);
        setRegion(region);
    ...
    public void checkRequest(Player player) {
        if (player.isHasBlueDiamond()) {
            player.setHasBlueDiamond(false);
