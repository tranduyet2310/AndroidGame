...
public class BlueTotem extends Enemy {
    public enum TotemState {IDLE, HIT, DEAD, ATTACK}

    public TotemState currentState;
    public TotemState previousState;
    ...
    public BlueTotem(PlayScreen screen, float x, float y) {
        super(screen, x, y);

        stateTimer = 0;
        currentHealth = maxHealth = Constants.SEASHELL_MAXHEALTH;
        powerAttack = Constants.SEASHELL_ATTACK;
        currentState = previousState = TotemState.IDLE;
        ...
        setBounds(getX(), getY(), 60 / Constants.PPM, 64 / Constants.PPM);
        velocity.x = 0.0f;
    ...
    public void update(float dt) {
        ...
        setPosition(b2body.getPosition().x - getWidth() / 2, b2body.getPosition().y - getHeight() / 2 + 16 / Constants.PPM);
    ...
    public TotemState getState() {
        if (isDead) {
            return TotemState.DEAD;
        } else if (isHurting) {
            return TotemState.HIT;
        } else if (isAttacking) {
            return TotemState.ATTACK;
        } else return TotemState.IDLE;
    }
    ...
    public void initAnimation(Array<TextureRegion> frames) {
        ...
        frames.add(new TextureRegion(Utils.getRegion("enemy/Totems/Head 1/Idle/1.png")));
        totemIdle = new Animation<TextureRegion>(0.4f, frames);
        frames.clear();
        for (int i = 1; i <= 6; i++) {
            frames.add(new TextureRegion(Utils.getRegion("enemy/Totems/Head 1/Attack/" + i + ".png")));
        }
        totemAttack = new Animation<TextureRegion>(0.4f, frames);
        frames.clear();
        for (int i = 1; i <= 6; i++) {
            frames.add(new TextureRegion(Utils.getRegion("enemy/Totems/Head 1/Destroyed/" + i + ".png")));
        }
        totemDead = new Animation<TextureRegion>(0.4f, frames);
        frames.clear();
        for (int i = 1; i <= 4; i++) {
            frames.add(new TextureRegion(Utils.getRegion("enemy/Totems/Head 1/Hit/" + i + ".png")));
        }
